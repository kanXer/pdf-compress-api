...
// --- 4. WORD TO PDF (REPLACED LIBREOFFICE) ---
async function wordToPDFAsync(inputPath) {
    const sessionID = crypto.randomUUID();
    const outputPath = path.join("outputs", `word_${sessionID}.pdf`);
    return new Promise((resolve, reject) => {
...
// --- 6. EXCEL TO PDF (REPLACED LIBREOFFICE) ---
async function excelToPDFAsync(inputPath) {
    const sessionID = crypto.randomUUID();
    const outputPath = path.join("outputs", `excel_${sessionID}.pdf`);
    ...
    doc.pipe(stream);
    doc.fontSize(12).text(`Worksheet: ${worksheet.name}`, { underline: true }).moveDown();
...
app.get("/check-setup", async (req, res) => {
    try {
        ...
        res.json({ ghostscript: gs.trim(), libreoffice: lo.trim(), poppler: pop.trim().split('\n')[0] });
